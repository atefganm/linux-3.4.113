	...
	struct bcmemac_platform_data *pd)
{
	...
	BDEV_UNSET(0xb80e18, 0x3f << 4);
	BDEV_SET(0xb80e18, 0x14 << 4);
#endif
...
static int __init platform_devices_setup(void)
{
	...
	/* Need to reverse the probe order */
	brcm_add_sdio_host(i++, BCHP_SDIO_1_CFG_REG_START,
		BCHP_SDIO_1_HOST_REG_START, BRCM_IRQ_SDIO1);
#endif /* defined(CONFIG_DREAMBOX) */

	brcm_add_sdio_host(i++, BCHP_SDIO_0_CFG_REG_START,
		BCHP_SDIO_0_HOST_REG_START, BRCM_IRQ_SDIO0);

#if !defined(CONFIG_DREAMBOX) && defined(BCHP_SDIO_1_CFG_REG_START)
	brcm_add_sdio_host(i++, BCHP_SDIO_1_CFG_REG_START,
		BCHP_SDIO_1_HOST_REG_START, BRCM_IRQ_SDIO1);
...
#if defined(CONFIG_DREAMBOX)
    #if defined(CONFIG_DREAMBOX_DM520)
	strcpy(board_info.modalias, "w25q128bv");
    #else
	strcpy(board_info.modalias, "mx25l12805d");
    #endif
#else
