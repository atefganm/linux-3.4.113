...
#include <linux/platform_data/adc128d818.h>
...
struct adc128_data {
	...
	unsigned int vlt_channels;	/* Number of voltage channels */
...
static int adc128_set_config_adv(struct adc128_data *data)
{
	unsigned int nchannels_lut[] = { 7, 8, 4, 6 };
	u8 regval;
	int err;
	...
	if (data->regulator)
		regval |= 0x01;
	err = i2c_smbus_write_byte_data(data->client, ADC128_REG_CONFIG_ADV, regval);
	data->vlt_channels = nchannels_lut[data->mode];
...
static ssize_t adc128_show_mode(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct adc128_data *data = adc128_update_device(dev);
...
static ssize_t adc128_set_mode(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t count)
{
	struct adc128_data *data = dev_get_drvdata(dev);
	...
	data->mode = val;
	err = adc128_set_config_adv(data);
	if (err < 0)
		return err;
...
static SENSOR_DEVICE_ATTR(mode, S_IWUSR | S_IRUGO, adc128_show_mode, adc128_set_mode, 0);
...
{
	struct adc128_platform_data *pdata;
