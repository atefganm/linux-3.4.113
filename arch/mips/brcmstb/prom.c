...
#if defined(CONFIG_DREAMBOX_DM7080) || defined(CONFIG_DREAMBOX_DM820)
#include <linux/initrd.h>

static inline unsigned long __init aon_read_blob_table(unsigned long idx)
{
	return BDEV_RD(AON_RAM_BASE + ((16 + idx) << 2));
}

static inline unsigned long __init aon_read_mac(unsigned long idx)
{
	return BDEV_RD(AON_RAM_BASE + ((126 + idx) << 2));
}

static void __init parse_blob_table(void)
{
	unsigned long len;
	unsigned long ptr;
	unsigned long i;

	if (aon_read_blob_table(0) != 0x6c6f6f70)
		return;

	len = aon_read_blob_table(1);
	if (len > 8) {
		printk(KERN_ERR "%s: Invalid length (%#lx), ignoring table\n", __func__, len);
		return;
	}

	ptr = 2;
	for (i = 0; i < len; i++) {
		unsigned long type = aon_read_blob_table(ptr++);
		unsigned long addr = aon_read_blob_table(ptr++);
		unsigned long size = aon_read_blob_table(ptr++);

		if (type == 0 || type == 0xFFFFFFFF ||
		    addr == 0 || addr == 0xFFFFFFFF ||
		    size == 0 || size == 0xFFFFFFFF) {
			printk(KERN_ERR "%s: Invalid entry, aborting\n", __func__);
			break;
		}

		if (addr >= BRCM_PCI_HOLE_START)
			addr += BRCM_PCI_HOLE_SIZE;

		if (addr >= PCIE_MEM_START) {
			printk(KERN_ERR "%s: Invalid address, aborting\n", __func__);
			break;
		}
		...
		case 4:
			...
			initrd_end = (unsigned long)__va(addr + size);
		#endif
			break;
		case 5:
			strlcpy(arcs_cmdline, __va(addr), COMMAND_LINE_SIZE);
			break;
		}
	}
}

static int parse_dreambox_mac_address(u8 *dest)
{
	unsigned long mac[2];

	mac[0] = aon_read_mac(0);
	mac[1] = aon_read_mac(1);

	dest[0] = 0x00;
	dest[1] = 0x09;
	dest[2] = 0x34;

	if (mac[0] != 9 || (mac[1] >> 24) != 0x34)
		mac[1] = 0xaabbcc;
...
#if defined(CONFIG_DREAMBOX_DM7080) || defined(CONFIG_DREAMBOX_DM820)
	parse_dreambox_mac_address(brcm_primary_macaddr);
#endif
	if (cfe_seal != CFE_EPTSEAL) {
		printk(KERN_CONT "none present, using defaults.\n");
#if defined(CONFIG_DREAMBOX_DM7080) || defined(CONFIG_DREAMBOX_DM820)
		parse_eth0_mdio_mode("1", &brcm_eth0_no_mdio);
		parse_string("RGMII_NO_ID", brcm_eth0_phy);
		parse_string("probe", brcm_eth0_phyaddr);
		parse_ulong("1000", &brcm_eth0_speed);
		parse_ulong("1024", &brcm_dram0_size_mb);
		parse_ulong("1024", &brcm_dram1_size_mb);
		parse_ulong("768", &brcm_dram1_linux_mb);
		parse_string("BCM97435B0", brcm_cfe_boardname);
		parse_ulong("16", &brcm_mtd_flash_size_mb);
...
static void __init init_port(unsigned int baud)
{
...
static void __init brcm_setup_early_printk(void)
{
	 ...
	 */
	while (arg && *arg != '\0' && *arg != ' ') {
		if ((*arg >= '0') && (*arg <= (field ? '9' : '3'))) {
			unsigned int digit = *arg - '0';
			if (field == 0 && digit < ARRAY_SIZE(base)) {
				dev = digit;
			} else if (field == 1) {
				baud *= 10;
				baud += digit;
